...
public final class BeanTableSchemaSubstitution {
    ...
    @Substitute
    private static <T> Supplier<T> newObjectSupplierForClass(Class<T> clazz, MethodHandles.Lookup lookup) {
        return BeanTableSchemaSubstitutionImplementation.newObjectSupplierForClass(clazz, lookup);
    }

    @Substitute
    private static <T, R> Function<T, R> getterForProperty(
            PropertyDescriptor propertyDescriptor, Class<T> beanClass, MethodHandles.Lookup lookup) {
        return BeanTableSchemaSubstitutionImplementation.getterForProperty(propertyDescriptor, beanClass, lookup);
    }

    @Substitute
    private static <T, U> BiConsumer<T, U> setterForProperty(
            PropertyDescriptor propertyDescriptor, Class<T> beanClass, MethodHandles.Lookup lookup) {
        return BeanTableSchemaSubstitutionImplementation.setterForProperty(propertyDescriptor, beanClass, lookup);
    }
}
